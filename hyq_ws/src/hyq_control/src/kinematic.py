#!/usr/bin/env python
import math
import hyq_constants as constants
import numpy as np

class my_Kinematic(object):

    def __init__(self):
        super(my_Kinematic, self).__init__()
    ...
    def TF_matrix(self,joint,d,a,alpha):
        TF = np.array([
            [math.cos(joint), -math.sin(joint)*math.cos(alpha), math.sin(joint)*math.sin(alpha), a * math.cos(joint)],
            [math.sin(joint), math.cos(joint)*math.cos(alpha), -math.sin(alpha)*math.cos(joint), math.sin(joint) * a],
    ...
    def inverse_kinematics(self,x,y,z,revers):
        ...
        k2 = z
        temp = (k1**2 + k2**2 - constants.lower_leg**2 - constants.upper_leg**2) / (2 * constants.upper_leg * constants.lower_leg)
        if temp>1: temp=1
        elif temp<-1 :temp=-1
        if revers:
    ...
    def print_FK(self,joints):
        ...
        print joints[1],joints[2],joints[-2],joints[-1]
        x, y, z = self.forward_kinematics_lf(joints[0], joints[1], joints[2])
        print 'lf-joint', x-constants.lf_init_position[0], y-constants.lf_init_position[1], z-constants.lf_init_position[2]

        x, y, z = self.forward_kinematics_lh(joints[3], joints[4], joints[5])
        #print 'lh-joint', x, y, z

        x, y, z = self.forward_kinematics_rf(joints[6], joints[7], joints[8])
        #print 'rf-joint', x, y, z

        x, y, z = self.forward_kinematics_rh(joints[9], joints[10], joints[11])
        print 'rh-joint', x-constants.rh_init_position[0], y-constants.rh_init_position[1], z-constants.rh_init_position[2]
    ...
    def calculate_shoulder(self,rpy_angle, h):
        ...
        lf_new_position = np.dot(
                [constants.lf_init_position[0], constants.lf_init_position[1], constants.lf_init_position[2] - h],
                np.dot(T1_lf, T2_lf))
        lh_new_position = np.dot(
                [constants.lh_init_position[0], constants.lh_init_position[1], constants.lh_init_position[2] - h],
                np.dot(T1_lf, T2_lf))
        rf_new_position = np.dot(
                [constants.rf_init_position[0], constants.rf_init_position[1], constants.rf_init_position[2] - h],
                np.dot(T1_lf, T2_lf))
        rh_new_position = np.dot(
                [constants.rh_init_position[0], constants.rh_init_position[1], constants.rh_init_position[2] - h],
                np.dot(T1_lf, T2_lf))
